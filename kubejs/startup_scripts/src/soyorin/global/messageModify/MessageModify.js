...
global.MessageModify = {
    ...
    init: function () {
        ForgeEvents.onEvent('net.minecraftforge.client.event.ClientChatReceivedEvent', event => {
            global.MessageModify.listenEvent(event);
        });
    },
    /**
    ...
    listenEvent: function (event) {
        ...
        const newMessage = global.MessageModify.modifyRawText(event); // 获取新消息
    ...
    register: function (pType, pPredicate, pFunction) {
        if (!global.MessageModify.registry[pType]) {
            global.MessageModify.registry[pType] = {
    ...
    modifyRawText: function (event) {
        ...
        const registry = global.MessageModify.registry;
...
const MessageModify = {
    // 初始化
    init: function () {
        console.log('Message 开始初始化!');
        global.MessageModify.init();
    },
    /**
     * 注册消息类型
     * @param {string} pType - 消息类型
     ...
     */
    register: function (pType, pPredicate, pFunction) {
        global.MessageModify.register(pType, pPredicate, pFunction);
    }
}
// 调用初始化
MessageModify.init();
